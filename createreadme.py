#!/usr/bin/env python3
"""
generate_readme.py — Генерирует README.md для проекта collectionerus-scraper.
Запуск: python generate_readme.py
Результат: README.md в текущей директории.
"""

import os

def main():
    # Плейсхолдер §§§ будет заменён на тройные обратные кавычки при записи
    content = r"""# 📦 Collectionerus Scraper
...
**Collectionerus Scraper** — инструмент для автоматического скачивания коллекций предметов (открытки, значки, марки, монеты и т.д.) с сайта collectionerus.ru. Проект состоит из трёх основных компонентов:
...
Проект создан для личного использования. Данные принадлежат сайту [collectionerus.ru](https://collectionerus.ru).
"""

    # Восстанавливаем тройные обратные кавычки
    ...
    print(f"✅ README.md создан: {size:,} байт")
